...
- Объявляет функцию `calculate_matrix_A`, которая принимает матрицу `A` размером `SIZE x SIZE`.
...
- Каждый элемент матрицы `A[i][j]` заполняется значением, вычисляемым по формуле (i−j)/(i+j+1)(i−j)/(i+j+1).
...
- Объявляет функцию `print_matrix`, принимающую строку `title` в качестве названия и матрицу `matrix` для отображения.
...
- Двы цикла проходят по всем элементам матрицы, выводя каждое значение с фиксированным форматом, выравненным по правому краю с двумя знаками после запятой.
- После вывода всех элементов каждой строки, функция печатает перевод строки.
...
- Печатает дополнительный перевод строки после вывода матрицы для лучшей организации вывода.
...
- Объявляет функцию `selection_sort`, принимающую массив `arr` и его размер `n`.
...
- Объявляет индексы и временную переменную для обмена, а также счетчик для подсчета присваиваний.
...
- Внешний цикл перебирает все элементы массива, а внутренний цикл ищет индекс минимального элемента.
- Если минимальный индекс отличается от текущего, элементы меняются местами, и счетчик присваиваний увеличивается на 3.
...
- Объявляет функцию `bubble_sort`, принимающую массив `arr` и его размер `n`.
...
- Объявляет индексы, временную переменную для обмена и счетчик для подсчета присваиваний.
...
- Внешний цикл перебирает массив, внутренний проверяет и обменивает соседние элементы, если они в неверном порядке. Счетчик присваиваний увеличивается на 3 при обмене.
...
- Объявляет функцию `insertion_sort`, принимающую массив `arr` и его размер `n`.
...
- Объявляет индексы, переменную `key` для текущего элемента и счетчик присваиваний.
...
- Внешний цикл проходит по массиву, внутренний — сдвигает элементы, которые больше `key`, чтобы найти нужную позицию для вставки. Счетчик присваиваний увеличивается.
...
#### Объявление и инициализация матриц и массива
...
- Объявление двухмерного массива `A` и одномерного массива `arr` с примерами значений для сортировки.
...
- Вызывается функция `calculate_matrix_A` для заполнения матрицы `A`.
...
- Вызывается функция `print_matrix` для отображения матрицы `A`.
...
- Вызывается функция `selection_sort`, и результат (количество присваиваний) выводится на экран.
...
- Вызывается функция `bubble_sort`, и результат (количество присваиваний) выводится на экран.

#### Заполнение массива для сортировки методом вставок
...
- Инициализация нового массива `arr_insertion` для тестирования сортировки методом вставок.
...
- Вызывается функция `insertion_sort`, и результат (количество присваиваний) выводится на экран.
